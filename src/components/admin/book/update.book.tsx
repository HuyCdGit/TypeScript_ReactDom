...
interface Iprops {
  isUpdateBookOpen: boolean;
  setIsUpdateBookOpen: (v: boolean) => void;
  refreshTable: () => void;
...
const UpdateBook = (props: Iprops) => {
  ...
  useEffect(() => {
    if (viewBook && viewBook._id) {
      const arrThumbNail = [
        {
          uid: uuidv1(),
          name: viewBook.thumbnail,
          status: "done",
          url: `${import.meta.env.VITE_BACKEND_URL}/images/book/${
            viewBook.thumbnail
          }`,
      ...
      const arrSlider = viewBook?.slider?.map((items) => {
        return {
          uid: uuidv1(),
          name: items,
          status: "done",
